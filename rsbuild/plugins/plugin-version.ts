  ...
  return {
    ...
    setup(build) {
      build.onAfterBuild(() => {
        ...
        fs.writeFileSync(path.resolve(build.context.distPath, 'version.json'), JSON.stringify(versionInfo, null, 2));
        console.log('[version]: version.json created successfully with version:', version);
      });
    },
  };
}
